...
export default function SessionDetails() {
  ...
  const fetchDetails = async () => {
    try {
      setLoading(true);
      setError('');
      const response = await fetch(`http://localhost:3001/api/agent/sessions/details/${sessionId}`, {
  ...
  const handleSaveNotes = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/agent/sessions/notes/${sessionId}`, {
        method: 'POST',
        headers: {
